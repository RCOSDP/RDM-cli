...
retrieve_project_obj = json.loads(retrieve_project_str, object_hook=lambda d: SimpleNamespace(**d))
...
def test_get_template_schema_projects():
...
def test_fake_project_content_data_verbose_is_false(capfd):
...
def test_fake_project_content_data_verbose_is_true(capfd):
...
def test_prepare_project_data_all_new_not_license(grdm_client):
...
def test_prepare_project_data_new_has_license_verbose_true(grdm_client, capfd):
...
def test_prepare_project_data_fork_project(grdm_client, capfd):
...
def test_load_project_is_fake_and_verbose_true(capfd, grdm_client):
...
def test_load_project_is_fake_false_and_request_error_and_ignore_error_false_sys_exit(mocker, grdm_client, capfd):
    error_message = 'error'
...
def test_load_project_is_fake_false_and_request_error_and_ignore_error_true(grdm_client, capfd):
    error_message = 'error'
    with mock.patch.object(grdm_client, '_request', return_value=(None, error_message)):
...
def test_load_project_is_fake_false_verbose_false(capfd, grdm_client):
    resp = requests.Response()
    resp._content = retrieve_project_str
    with mock.patch.object(grdm_client, '_request', return_value=(resp, None)):
        ...
        assert actual1 == retrieve_project_obj.data
        assert actual2 == json.loads(retrieve_project_str)['data']


@mock.patch('sys.exit', side_effect=Exception())
def test_fork_project_request_error_and_ignore_error_false_sys_exit(mocker, grdm_client, capfd):
...
def test_fork_project_request_error_and_ignore_error_true(grdm_client, capfd):
    error_message = 'error'
    _node_project = projects['projects'][3]
    with mock.patch.object(grdm_client, '_request', return_value=(None, error_message)):
...
def test_fork_project_verbose_true(grdm_client, capfd):
    resp = requests.Response()
...
@mock.patch('sys.exit', side_effect=Exception())
def test_create_project_request_error_and_ignore_error_false_sys_exit(mocker, grdm_client, capfd):
...
def test_create_project_request_error_and_ignore_error_true(grdm_client, capfd):
    error_message = 'error'
    _node_project = projects['projects'][0]
    with mock.patch.object(grdm_client, '_request', return_value=(None, error_message)):
...
def test_create_project_verbose_true(grdm_client, capfd):
    resp = requests.Response()
...
@mock.patch('sys.exit', side_effect=Exception())
def test_link_project_to_project_request_error_and_ignore_error_false_sys_exit(mocker, grdm_client, capfd):
...
def test_link_project_to_project_request_error_and_ignore_error_true(grdm_client, capfd):
    error_message = 'error'
    _project_id = projects['projects'][2]['id']
    with mock.patch.object(grdm_client, '_request', return_value=(None, error_message)):
...
def test_link_project_to_project_verbose_true(grdm_client, capfd):
    resp = requests.Response()
...
def test_add_project_pointers_project_link_is_none(grdm_client, capfd):
...
def test_create_or_load_project_case_load_project_none(grdm_client, capfd):
...
def test_create_or_load_project_case_load_project(capfd, grdm_client):
...
def test_create_or_load_project_case_create_project_none(capfd, grdm_client):
    _projects = (projects.get('projects', [])).copy()
...
def test_create_or_load_project_case_create_project(capfd, grdm_client):
    _projects = projects.get('projects', [])
...
def test_create_or_load_project_case_fork_project_none(capfd, grdm_client):
    _projects = (projects.get('projects', [])).copy()
...
def test_create_or_load_project_case_fork_project(capfd, grdm_client):
    _projects = projects.get('projects', [])
    _fork_id = _projects[3].get('fork_id')
...
def test_add_project_components_children_exist_id_ignored(grdm_client, capfd):
...
def test_add_project_components_add_components_is_none(grdm_client, capfd):
...
def test_add_project_components_add_components_success(grdm_client, capfd):
...
@mock.patch('sys.exit', side_effect=Exception())
def test_projects_add_component_request_error_and_ignore_error_false_sys_exit(mocker, grdm_client, capfd):
...
def test_projects_add_component_request_error_and_ignore_error_true(grdm_client, capfd):
    error_message = 'error'
    _project = projects['projects'][2]
    parent_id = 'nid11'
    with mock.patch.object(grdm_client, '_request', return_value=(None, error_message)):
...
def test_projects_add_component_verbose_true(grdm_client, capfd):
    resp = requests.Response()
    resp._content = new_project_str
    _project = projects['projects'][2]
    parent_id = 'nid11'
    _project['project_links'][1] = None
...
@mock.patch('sys.exit', side_effect=Exception())
def test_projects_create_schema_not_exist(mocker, grdm_client):
    with mock.patch('os.path.exists', return_value=False):
        with pytest.raises(Exception):
            projects_create(grdm_client)
        mocker.assert_called_once_with(f'Missing the template schema {grdm_client.template_schema_projects}')


@mock.patch('sys.exit', side_effect=Exception())
def test_projects_create_template_file_not_exist(mocker, grdm_client):
...
def test_projects_create_verbose_false_check_schema_raise_error(mocker, grdm_client, capfd):
...
@mock.patch('sys.exit')
def test_projects_create_verbose_true(mocker, grdm_client, capfd):
...
@mock.patch('sys.exit')
def test_projects_create_case_create_or_load_project_none(mocker, grdm_client, capfd):
    _projects = {"projects": [projects['projects'][2]]}
    mocker.patch('grdmcli.utils.write_json_file')
    mocker.patch('grdmcli.utils.check_json_schema')
    mocker.patch('os.path.exists', side_effect=[True, True])
    with mock.patch('grdmcli.utils.read_json_file', return_value=_projects):
        with mock.patch.object(grdm_client, '_create_or_load_project',
                               return_value=test_create_or_load_project_case_load_project_none(grdm_client, capfd)):
            projects_create(grdm_client, verbose=False)
            lines = capfd.readouterr().out.split('\n')
            assert lines[4] == f'USE the output result file: {grdm_client.output_result_file}'
